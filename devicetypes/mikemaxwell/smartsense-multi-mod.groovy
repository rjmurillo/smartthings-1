metadata {
	...
	definition (name: "SmartSense Multi (mod)", namespace: "MikeMaxwell", author: "SmartThings") {
	...
	preferences {
        ...
        //input "isAuto","boolean", title: "Auto offset enable.", description: "Enable axis auto zero when contact is closed."
    }
...
private Map parseSingleMessage(description) {
	...
	//here
    log.info "getContactResult- value:${value} isStateChange:${isStateChange} isAuto:${isAuto}"
    //if (value == "closed" && isStateChange &&  isAuto) {
    //	setAxisZero()
    //}
	...
	log.debug "ParseSingle for $device: $results"
	results
}
...
private Map parseOtherMessage(description) {
	...
	log.debug "ParseOther for $device: $results"
...
def setAxisZero() {
    def xyzResults = device.currentValue("threeAxis")
    log.info "setAxisZero crntValue:${xyzResults}"
	state.xOffset = xyzResults.x ?: 0
    state.yOffset = xyzResults.y ?: 0
    state.zOffset = xyzResults.z ?: 0
	log.info "setAxisZero offsets:${state}"
	sendEvent(name: "threeAxis", value:"0,0,0")
}
...
private List parseOrientationMessage(String description) {
	...
	parts.each { part ->
		...
		if (part.startsWith('x:')) {
            ...
            //if (state.xOffset) xyzResults.x = signedX - state.xOffset.toInteger() 
            //else xyzResults.x = signedX
            xyzResults.x = signedX
		}
		else if (part.startsWith('y:')) {
			def unsignedY = part.split(":")[1].trim().toInteger()
			def signedY = unsignedY > 32767 ? unsignedY - 65536 : unsignedY
            //if (state.yOffset) xyzResults.y = signedY - state.yOffset.toInteger()
            //else xyzResults.y = signedY
            xyzResults.y = signedY
		}
		else if (part.startsWith('z:')) {
			def unsignedZ = part.split(":")[1].trim().toInteger()
			def signedZ = unsignedZ > 32767 ? unsignedZ - 65536 : unsignedZ
            //if (state.zOffset) xyzResults.z = signedZ - state.zOffset.toInteger() 
            //else xyzResults.z = signedZ
